...
public class MyAlgoLogic implements AlgoLogic {
    ...
    public Action evaluate(SimpleAlgoState state) {
        ...
        if (!activeOrders.isEmpty() && totalOrderCount >= MAX_ORDER_COUNT) {
            ChildOrder oldestOrder = activeOrders.get(0); // Cancel the oldest active order
            logger.info("[MYALGO] Cancelling oldest active order: " + oldestOrder);
            return new CancelChildOrder(oldestOrder); // Return action to cancel the order
        }

        // Now handle order matching logic
